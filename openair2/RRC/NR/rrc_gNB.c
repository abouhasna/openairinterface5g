      ...
      case NR_UL_CCCH_MessageType__c1_PR_rrcSetupRequest:
        LOG_D(NR_RRC, "Received RRCSetupRequest on UL-CCCH-Message (UE rnti %04x)\n", rnti);
        {
	      ...
	      gNB_RRC_UE_t *UE = &ue_context_p->ue_context;
              ...
              if (UE->rnti != rnti){
                uint64_t random_value = 0;
                memcpy(((uint8_t *)&random_value) + 3, rrcSetupRequest->ue_Identity.choice.randomValue.buf,
                ...
                LOG_E(NR_RRC, "RRCSetupRequest RANDOM value match case not supported, let's reject the UE %04x\n", rnti);
	      ...
	      if (UE->rnti != rnti){
                 ...
                 PROTOCOL_CTXT_SET_BY_INSTANCE(&ctxt, module_id, GNB_FLAG_YES, UE->rrc_ue_id, 0, 0);
...
static void rrc_CU_process_ue_context_release_complete(MessageDef *msg_p)
{
  ...
  rrc_gNB_send_NGAP_UE_CONTEXT_RELEASE_COMPLETE(instance, ue_context_p);
