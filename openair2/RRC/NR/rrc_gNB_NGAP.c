...
void rrc_gNB_send_NGAP_UE_CONTEXT_RELEASE_COMPLETE(
  instance_t instance,
  rrc_gNB_ue_context_t     *const ue_context_p) {
  ...
  for (int i = 0; i < ue_context_p->ue_context.nb_of_pdusessions; i++) {
      NGAP_UE_CONTEXT_RELEASE_COMPLETE(msg).pdusessions[i].pdusession_id = ue_context_p->ue_context.pduSession[i].param.pdusession_id;
...
//------------------------------------------------------------------------------
{
  ...
  PROTOCOL_CTXT_SET_BY_INSTANCE(&ctxt, instance, GNB_FLAG_YES, UE->rrc_ue_id, 0, 0);
  LOG_I(
      NR_RRC, "PDU Session Release Command: AMF_UE_NGAP_ID %lu  rrc_ue_id %u release_pdusessions %d gNB UE context nb_of_pdusession %d\n",
       cmd->amf_ue_ngap_id, gNB_ue_ngap_id, cmd->nb_pdusessions_torelease,UE->nb_of_pdusessions);
  bool found = false;
