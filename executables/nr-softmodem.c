...
#include "openair2/COLLECTOR/collector.h"
...
static int create_gNB_tasks(void) {
  ...
  if (gnb_nb > 0) {
    ...
    LOG_I(NR_RRC, "COLLECTOR: Creating NR COLLECTOR gNB Task, that will also create TASKS\n");
    if (itti_create_task(TASK_COLLECTOR, NR_Collector_Task, NULL) < 0) {
          LOG_E(NR_RRC, "Create task for NR Collector is failed\n");
